#!/usr/bin/env python3
"""
Jito 专属适配器 — data/protocols/jito/adapter.py
...
- 不回购的钱进国库/支出 → 留存行标注去向
- MEV 收益 96% 归 JitoSOL 持有人 + 4% Jito Foundation，JTO 不分润
...
def build_snapshot(proto_dir):
    ...
    gp = {
        ...
        "calculation_note": "DefiLlama dailyRevenue 为协议净收入口径（MEV 分润给 JitoSOL 部分已扣），毛利 = 收入",
    }
    emission = {
        "usd_365d": None,
        ...
        "treatment": "none",
        "calculation_note": "JTO 供应上限 10 亿（流通 4.3 亿，持续 unlock 为无对价稀释），无对价收入模型，不涉及增发成本扣减",
    }
    net_income = {
        "net_income_usd_365d": revenue,
        "operating_cost_usd_365d": None,
        "calculation_note": "净利 = 收入 − 增发(0) − 运营成本(数据不可得)；净利留存 Jito Foundation/支出，股东回报 = 0（治理代币）",
    }
    ...
    by_mechanism = [
        {
            ...
            "yield_percent": 0,
            "verified": "estimated",
            "note": "治理代币，无股东回报（不回购/不分红/不销毁）；MEV 收益 96% 归 JitoSOL 持有人 + 4% Jito Foundation，JTO 不分润",
        }
    ]
    holder_returns = {
    ...
    return {
        ...
        "income_statement": {
            "revenue": {
                ...
                "revenue_excluded": {
                    "mev_to_jitosol": {
                        "note": "MEV tips 96% 归 JitoSOL 持有人（~5% APY 含 staking+MEV），JTO 不分润；JTO staking 收入分成仍为 planned"
                    },
                },
        ...
        "verification": {
            "method": f"净利 = DefiLlama dailyRevenue 365d ${revenue:,.0f}（协议净收入）；"
                      f"JTO 治理代币不回购（MEV 归 JitoSOL）；净利留存 Foundation；JTO staking 收入分成 planned",
            "status": "estimated",
...
if __name__ == "__main__":
    snap = build_snapshot(BASE_DIR / "data" / "protocols" / "jito")
