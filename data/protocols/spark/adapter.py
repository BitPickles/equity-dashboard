#!/usr/bin/env python3
"""
Spark 专属适配器 — data/protocols/spark/adapter.py
...
- 不回购的钱进国库 → 留存行标注去向；Spark 收入流向 Sky 主 DAO（Smart Burn Engine 回购 SKY，非 SPK）
...
def build_snapshot(proto_dir):
    ...
    emission = {
        ...
        "treatment": "none",
        "calculation_note": "SPK 为 Sky SubDAO 代币（2024-Q3 启动），存在排放但无对价收入模型，不涉及增发成本扣减",
    }
    net_income = {
        "net_income_usd_365d": revenue,
        "operating_cost_usd_365d": None,
        "calculation_note": "净利 = 收入 − 增发(0) − 运营成本(数据不可得)；Spark 收入流向 Sky 主 DAO，SPK 本身股东回报 = 0（治理代币）",
    }
    ...
    by_mechanism = [
        {
            ...
            "yield_percent": 0,
            "verified": "estimated",
            "note": "治理代币，无股东回报（不回购/不分红/不销毁）；Spark 收入归 Sky 主 DAO（Smart Burn Engine 回购 SKY），SPK 仅 SubDAO 治理",
        }
    ]
    holder_returns = {
    ...
    return {
        ...
        "income_statement": {
            "revenue": {
                ...
                "revenue_excluded": {
                    "flow_to_sky_dao": {
                        "note": "Spark 收入流向 Sky 主 DAO（Smart Burn Engine 回购 SKY），非 SPK 持有人；SPK 无股东回报"
                    },
                },
        ...
        "verification": {
            "method": f"净利 = DefiLlama dailyRevenue 365d ${revenue:,.0f}（协议净收入）；"
                      f"SPK 治理代币不回购——收入流向 Sky 主 DAO 回购 SKY；SPK 股东回报 = 0",
            "status": "estimated",
...
if __name__ == "__main__":
    snap = build_snapshot(BASE_DIR / "data" / "protocols" / "spark")
