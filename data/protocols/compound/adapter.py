#!/usr/bin/env python3
"""
Compound 专属适配器 — data/protocols/compound/adapter.py
...
- 不回购的钱进国库/协议储备 → 留存行标注去向
- 借贷 fees ($180M+/年) 100% 给 LP，协议端收入很小（fee switch OFF）
...
def build_snapshot(proto_dir):
    pid = Path(proto_dir).name  # "compound"
    ...
    # 借贷 fees 大部分给 LP，协议端 revenue 仅 reserve factor 部分（fee switch OFF）
    ...
    gp = {
        ...
        "calculation_note": "DefiLlama dailyRevenue 为协议净收入口径（LP 分润已在数据源扣减），毛利 = 收入",
    }
    emission = {
        "usd_365d": None,
        ...
        "treatment": "none",
        "calculation_note": "COMP 供应上限 1000 万（接近全流通），无持续增发成本",
    }
    net_income = {
        "net_income_usd_365d": revenue,
        "operating_cost_usd_365d": None,
        "calculation_note": "净利 = 收入 − 增发(0) − 运营成本(数据不可得)；净利留存协议储备，股东回报 = 0（治理代币）",
    }
    ...
    by_mechanism = [
        {
            ...
            "yield_percent": 0,
            "verified": "estimated",
            "note": "治理代币，无股东回报（不回购/不分红/不销毁）；fee switch 从未开启，借贷 fees 100% 给 LP",
        }
    ]
    holder_returns = {
    ...
    return {
        ...
        "income_statement": {
            "revenue": {
                ...
                "revenue_excluded": {
                    "lending_fees_to_lp": {
                        "note": "借贷 fees (~$180M+/年) 100% 给 LP；协议端 reserve factor 收入很小（fee switch OFF，多次治理讨论未通过）"
                    },
                },
        ...
        "valuation": valuation,
        "verification": {
            "method": f"净利 = DefiLlama dailyRevenue 365d ${revenue:,.0f}（协议净收入，reserve factor 口径）；"
                      f"COMP 治理代币不回购（fee switch OFF）；净利留存协议储备",
            "status": "estimated",
...
if __name__ == "__main__":
    snap = build_snapshot(BASE_DIR / "data" / "protocols" / "compound")
