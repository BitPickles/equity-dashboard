#!/usr/bin/env python3
"""
Morpho 专属适配器 — data/protocols/morpho/adapter.py
...
- 当前协议收入 = 0（Morpho Blue 协议层不抽成，vault curators 自行设 fee）
...
def build_snapshot(proto_dir):
    pid = Path(proto_dir).name  # "morpho"
    ...
    # Morpho Blue 是 immutable primitive，协议层不抽成 → 协议端 dailyRevenue = 0
    revenue = metrics.get("trailing_365d_revenue_usd")  # 0（协议层无 fee）

    # ── 毛利 / 增发 / 净利 ─────────────────────────────────────
    gp = {
        "lp_share_cost_usd_365d": None,
        "gross_profit_usd_365d": revenue,
        "calculation_note": "DefiLlama dailyRevenue 为协议净收入口径；当前协议层不抽成（vault curators 自行设 fee），毛利 = 0",
    }
    emission = {
        "usd_365d": None,
        ...
        "treatment": "none",
        "calculation_note": "MORPHO 存在解锁/排放（无对价），但当前无协议收入模型，不涉及增发成本扣减",
    }
    net_income = {
        "net_income_usd_365d": revenue,
        "operating_cost_usd_365d": None,
        "calculation_note": "净利 = 收入 − 增发(0) − 运营成本(数据不可得)；协议层无收入（fee switch OFF），股东回报 = 0（治理代币）",
    }
    ...
    by_mechanism = [
        {
            ...
            "yield_percent": 0,
            "verified": "estimated",
            "note": "治理代币，无股东回报（不回购/不分红/不销毁）；Morpho Blue 协议层不抽成，fees 100% 归 vault LP/curator",
        }
    ]
    holder_returns = {
    ...
    margins = {
        ...
        "note": "派生计算：收入为 0（协议层不抽成），毛利率/净利率无意义（null）",
    }
    ...
    return {
        ...
        "income_statement": {
            "revenue": {
                ...
                "revenue_excluded": {
                    "vault_curator_fees": {
                        "note": "借贷 fees (~$180M/年) 100% 归 vault LP/curator；Morpho Blue 协议层不抽成（fee switch OFF）"
                    },
                },
        ...
        "valuation": valuation,
        "verification": {
            "method": "DefiLlama dailyRevenue 365d = $0（Morpho Blue 协议层不抽成，vault curators 自行设 fee）；"
                      "MORPHO 治理代币不回购（fee switch OFF）",
            "status": "estimated",
...
if __name__ == "__main__":
    snap = build_snapshot(BASE_DIR / "data" / "protocols" / "morpho")
