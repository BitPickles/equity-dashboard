#!/usr/bin/env python3
"""
EigenLayer 专属适配器 — data/protocols/eigenlayer/adapter.py
...
- 当前协议收入 = 0（AVS 费用全流向 operator/restaker，EIGEN 无收入分配权）
...
def build_snapshot(proto_dir):
    pid = Path(proto_dir).name  # "eigenlayer"
    ...
    # EigenLayer AVS 费用全流向 operator/restaker（持 ETH/LST），协议端 dailyRevenue = 0
    revenue = metrics.get("trailing_365d_revenue_usd")  # 0（协议端无收入）

    # ── 毛利 / 增发 / 净利 ─────────────────────────────────────
    gp = {
        "lp_share_cost_usd_365d": None,
        "gross_profit_usd_365d": revenue,
        "calculation_note": "DefiLlama dailyRevenue 为协议净收入口径；当前 AVS 费用全流向 operator/restaker，协议端毛利 = 0",
    }
    emission = {
        "usd_365d": None,
        ...
        "treatment": "none",
        "calculation_note": "EIGEN 存在 unlock/再质押排放（无对价），但当前无协议收入模型，不涉及增发成本扣减",
    }
    net_income = {
        "net_income_usd_365d": revenue,
        "operating_cost_usd_365d": None,
        "calculation_note": "净利 = 收入 − 增发(0) − 运营成本(数据不可得)；协议无收入（AVS fee 归 operator/restaker），股东回报 = 0（治理代币）",
    }
    ...
    by_mechanism = [
        {
            ...
            "yield_percent": 0,
            "verified": "estimated",
            "note": "治理代币，无股东回报（不回购/不分红/不销毁）；AVS 费用全流向 operator/restaker，EIGEN 无收入分配权",
        }
    ]
    holder_returns = {
    ...
    margins = {
        ...
        "note": "派生计算：收入为 0（协议端无收入），毛利率/净利率无意义（null）",
    }
    ...
    return {
        ...
        "income_statement": {
            "revenue": {
                ...
                "revenue_excluded": {
                    "avs_fees": {
                        "note": "AVS 费用全流向 operator/restaker（持 ETH/LST），EIGEN 无收入分配权；fee switch OFF"
                    },
                },
        ...
        "valuation": valuation,
        "verification": {
            "method": "DefiLlama dailyRevenue 365d = $0（AVS 费用流向 operator/restaker，协议端无收入）；"
                      "EIGEN 治理代币不回购（fee switch OFF）；restaking rewards 全流向 validator",
            "status": "estimated",
...
if __name__ == "__main__":
    snap = build_snapshot(BASE_DIR / "data" / "protocols" / "eigenlayer")
