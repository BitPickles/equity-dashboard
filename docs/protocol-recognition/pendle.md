...
1. 读 config.json → `tevRatio = 0.8`（判定书：80% 确定）；all-protocols.json → `metrics.trailing_365d_revenue_usd`（dailyRevenue = $18,665,264）+ `trailing_365d_tev_usd`（dailyHoldersRevenue = $18,665,264）
...
- 配置文件：`data/protocols/pendle/config.json`（revenue_recognition 字段，tevRatio=0.8）
