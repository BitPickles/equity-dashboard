...
> Boss 测试站验收后提出 UI 优化需求。**以下为全项目 UI 标准**，M2 批量接入时按此渲染。
...
- **可拖动**：chartjs-plugin-zoom 启用 `pan: { mode: 'x' }`（拖动平移时间轴）
- **可缩放**：`zoom: { wheel: { enabled: true } }`（滚轮缩放）+ pinch 触屏
- **双击重置**视图（plugin-zoom 默认）
- **纵坐标简写**：B=十亿 / M=百万 / K=千（y 轴 tick callback）
- **横坐标时间取整**：显示「2026年6月」（月份粒度），跨年带年份；不显示具体日（"6月11号"这类噪音不出现）
- **指标切换按钮在图表下方**（pill 风格：圆角胶囊按钮，active 绿色高亮），不在上方
